...
class DBController:
    ...
    def create_user(c, self, username, credits=0):
        c.execute('INSERT INTO users (username, credits) VALUES (?, ?)', (username, credits))
    ...
    def create_item(c, self, user_id, name, price):
        ...
        c.execute('INSERT INTO items (user_id, name, price) VALUES (?, ?, ?)', (user_id, name, price))
    ...
    def get_items(c, self, user_id):
        ...
        return [{'id': item[0], 'user_id': item[1], 'name': item[2], 'price': item[3]} for item in items]

    @_connect_decorator
    def get_item_with_name(c, self, user_id, item_name):

        c.execute('SELECT * FROM items WHERE user_id = ? AND name = ?', (user_id, item_name))
        items = c.fetchall()
