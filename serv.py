...
class Metagameplayer(metagameplay_pb2_grpc.Metagameplayer):
    ...
    def SayHello(self, request, context):
        return metagameplay_pb2.HelloReply(message=config.HELLO_STR_S %
                                           request.name)

    def Login(self, user, context):
        if user.nickname == '':
            return metagameplay_pb2.SessionResponse(
                status=metagameplay_pb2.Status(ok=False,
        ...
        return metagameplay_pb2.SessionResponse(
            user=user, status=metagameplay_pb2.Status(ok=True))
    ...
    def Logout(self, user, context):
        ...
        return metagameplay_pb2.SessionResponse(status=metagameplay_pb2.Status(
            ok=True))
    ...
    def GetAllItems(self, user, context):
        ...
        resp = metagameplay_pb2.ItemsResponse(status=metagameplay_pb2.Status(
            ok=True))
...
def serve():
    ...
    metagameplay_pb2_grpc.add_MetagameplayerServicer_to_server(
        Metagameplayer(), server)
